...
- **Menu** : `KV_Menu`, ouvert par une **étoile du Nether en emplacement 4**. Dans un plot validé pas encore noté,
  ...
  **« Voter »** du menu.
- **Informations du plot** : dans un plot, tableau sur le côté (titre, créateur, éditeurs, état, note globale, votes,
  points, note donnée) ; seule la description est écrite dans le chat.
