...
| **Kal-Games** (`kal-games`, hub, machine 7001, ex KalGames2) | copie complète de kal-games du 24/09/2026, puis (LeKiwi06) : **`KalGames-1.18.0.jar`** (types de mini-jeux ouverts, éditeur de checkpoints) + **`KG_BoatRace-1.4.1.jar`** (course de bateau sortie de KalGames) + **`KG_ScoreBoards-1.4.0.jar`** (journal des parties, points décimaux) + `KG_Menu-1.0.0.jar` + `KLM_Menu-2.1.0.jar` + `KG_Bingo-1.3.0.jar` (`bingo.server-name: serveur-jeux`) ; anciens jars et configs dans les `_removed-…` de chaque plugin ; + Floodgate, BedrockSkinRestorer, VelocityCommandForward, GrimAC | testé et confirmé par LeKiwi06 les 24 et 25/09/2026 (menus, boussole, course de bateau : checkpoints, km/h, classement en direct, écarts, barème, hors-piste, anti-collision Java et Bedrock ; Parkour, classements, Bingo) ; non testé : Rush, capture d'arène |
...
- **KG_Parkour** (cahier des charges : `KG_Parkour/CAHIER_DES_CHARGES.md`) : à sortir de KalGames (comme la course),
  ...
  (Mannequin), anti-collision entre joueurs. Limite des 5 parties privées par jour à supprimer pour tous les jeux.
...
- **KG_ScoreBoards** : journal des parties (1.3.0) et points décimaux (1.4.0) faits. Restent l'étape B du Bingo
