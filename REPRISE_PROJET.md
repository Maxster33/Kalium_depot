...
| Kanvas (ex Kal-Test-Dev) | `KaliumCore-1.4.0.jar` + `KLM_Menu-2.0.0.jar` (24/09/2026, rôle `backend` par défaut) ; `KV_Plots-1.0.0.jar` (26/09/2026, LeKiwi06) ; FastAsyncWorldEdit 2.15.4 + WorldGuard 7.0.19 (installés par LeKiwi06) ; monde principal renommé `Kanvas` (`level-name=Kanvas`) | KV_Plots non testé (redémarrage à faire par l'humain) ; KaliumCore non testé (projet en pause) |
