...
| Kanvas (ex Kal-Test-Dev) | `KaliumCore-1.4.0.jar` + `KLM_Menu-2.0.0.jar` (24/09/2026, rôle `backend` par défaut) ; `KV_Plots-1.0.0.jar` (26/09/2026, LeKiwi06) ; FastAsyncWorldEdit **Paper** 2.15.4 (26/09/2026 : la variante Bukkit/Spigot installée d'abord ne fonctionnait pas sur Paper 26.2, rangée dans `_removed-fastasyncworldedit-2.15.4-bukkit/`) + WorldGuard 7.0.19 ; monde principal renommé `Kanvas` (`level-name=Kanvas`) | KV_Plots testé et confirmé par LeKiwi06 le 26/09/2026 ; KaliumCore non testé (projet en pause) |
