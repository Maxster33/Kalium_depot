...
| **Kal-Games** (`kal-games`, hub, machine 7001, ex KalGames2) | copie complète de kal-games du 24/09/2026, puis (LeKiwi06) : **`KalGames-1.19.1.jar`** (types de mini-jeux ouverts, éditeur de checkpoints, orientation de la caméra gardée aux checkpoints, plus de limite de parties privées, attributions journalisées) + **`KG_BoatRace-1.4.1.jar`** (course de bateau sortie de KalGames) + **`KG_ScoreBoards-1.5.0.jar`** (journal des parties, points décimaux, `/classements verifier` et `crediter`) + `KG_Menu-1.0.0.jar` + `KLM_Menu-2.1.0.jar` + `KG_Bingo-1.4.0.jar` (`bingo.server-name: serveur-jeux` ; pseudo de l'hôte corrigé dans la liste) ; anciens jars et configs dans les `_removed-…` de chaque plugin ; + Floodgate, BedrockSkinRestorer, VelocityCommandForward, GrimAC | testé et confirmé par LeKiwi06 les 24 et 25/09/2026 (menus, boussole, course de bateau : checkpoints, km/h, classement en direct, écarts, barème, hors-piste, anti-collision Java et Bedrock ; Parkour, classements, Bingo) ; non testé : Rush, capture d'arène |
...
| Kanvas (ex Kal-Test-Dev) | `KaliumCore-1.4.0.jar` + `KLM_Menu-2.0.0.jar` (24/09/2026, rôle `backend` par défaut) ; `KV_Plots-1.2.0.jar` + `KV_Menu-1.1.0.jar` (26/09/2026 02:37, LeKiwi06 ; anciennes dans `_removed-kv_plots-1.1.0/`, `_removed-kv_plots-1.1.1/`, `_removed-kv_menu-1.0.0/` ; `_removed-kv_plots-1.0.0/` en trop, à supprimer par l'humain) ; FastAsyncWorldEdit **Paper** 2.15.4 (26/09/2026 : la variante Bukkit/Spigot installée d'abord ne fonctionnait pas sur Paper 26.2, rangée dans `_removed-fastasyncworldedit-2.15.4-bukkit/`) (limites `limits.default` réduites le 26/09/2026, original dans `_removed-fastasyncworldedit-config-2026-09-26/`) + WorldGuard 7.0.19 ; monde principal renommé `Kanvas` (`level-name=Kanvas`) | KV_Plots 1.0.0 testé et confirmé par LeKiwi06 le 26/09/2026 ; FAWE Paper testé et confirmé par LeKiwi06 le 26/09/2026 (joueur : seulement dans son plot, sélection à cheval sur la bordure limitée au plot ; permissions LuckPerms du groupe `default` données ; baguette de navigation de WorldEdit passée de la boussole au vide de structure, original dans `_removed-fastasyncworldedit-config-2026-09-26/`) ; KV_Plots 1.2.0 et KV_Menu 1.1.0 testés et confirmés par LeKiwi06 le 26/09/2026 (créatif gardé, validation, votes, rangement étoile / boussole) ; KaliumCore non testé (projet en pause) |
