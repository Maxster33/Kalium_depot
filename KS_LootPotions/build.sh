#!/bin/sh
# Compile KS_LootPotions (ECJ, cible Java 21 : tourne sur Java 21+ / 25) et assemble le .jar
...
OUT="$TOOLS/classes/KS_LootPotions"
...
jar cf "$DEST/KS_LootPotions-$VERSION.jar" -C "$OUT" .
echo "OK -> $DEST/KS_LootPotions-$VERSION.jar"
